...
GString *g_string_replace(GString *str, const gchar *old, const gchar *new) {
    gchar *pos;
    GString *result = g_string_new(NULL);
    gsize old_len = strlen(old);
    gsize new_len = strlen(new);
    gsize start = 0;

    while ((pos = strstr(str->str + start, old)) != NULL) {
        g_string_append_len(result, str->str + start, pos - (str->str + start));
        g_string_append_len(result, new, new_len);
        start = pos - str->str + old_len;
    }

    g_string_append(result, str->str + start);
    return result;
}
...
void video_renderer_init(logger_t *render_logger, const char *server_name, videoflip_t videoflip[2], const char *parser,
	...
	if (!strcmp(renderer_type[i]->codec, h265)) {
            GString *new_string = g_string_replace (launch, (const gchar *) h264,  (const gchar *) h265);
            g_string_free(launch, TRUE); // Free the old string if necessary
            launch = new_string; // Assign the new string
	} else {
            GString *new_string = g_string_replace (launch, (const gchar *) h265,  (const gchar *) h264);
            g_string_free(launch, TRUE); // Free the old string if necessary
            launch = new_string; // Assign the new string
	}
